...
const maxEnemyHealth: number = 50;
const enemyAttackDamage: number = 10;

// Player variables
let health: number = 100;
const attackDamage: number = 20;
...
while (running) {
  ...
  let enemyHealth: number = Math.floor(Math.random() * 75);
  const enemy: string = enemies[Math.floor(Math.random() * 4)];
